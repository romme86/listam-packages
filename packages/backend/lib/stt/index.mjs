...
//
// 'whisper-cpp' on the Node headless host. 'whisper-server' (also Node) keeps a
// warm whisper.cpp server so the model loads once instead of per utterance.
// 'whisper-bare' on the desktop Pear worker (same whisper-cli, spawned via
// injected bare-subprocess/bare-fs). A 'fixture' engine (config.transcribe
// injected) keeps the voice pipeline end-to-end testable without a real model.
// `runtime` carries the Bare modules the worker injects ({ subprocess, fs,
// tmpDir }); ignored by the Node engines. Engines may expose an optional
// stop() for process cleanup; hosts call it on shutdown when present.

import { createWhisperCppStt } from './whisper-cpp-subprocess.mjs'
import { createWhisperServerStt } from './whisper-server.mjs'
...
export function createStt ({ engine = 'whisper-cpp', config = {}, logger = null, runtime = {} } = {}) {
    switch (engine) {
        ...
        case 'whisper-server':
            return createWhisperServerStt({ config, logger })
        case 'whisper-bare':
