...
// Single-list sharing (multi-base). SHARE_LIST { listId, type?, name? } promotes
// a list into its OWN base and returns { ok, invite, baseKey, listId }. `type`
// disambiguates a legacy multiplexed built-in; only default:shopping is eligible,
// and it is re-IDed so recipients mount it alongside their own default grocery.
// JOIN_LIST { invite } ADDITIVELY joins a shared list's base (unlike RPC_JOIN_KEY,
// which replaces the whole personal base). Both reply request/response.
